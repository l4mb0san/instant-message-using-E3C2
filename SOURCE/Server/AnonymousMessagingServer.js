...
io.on('connection', function (socket) {
	...
	//Tham vào vào room monitor
	socket.on('join-room-monitor', function(){
		socket.join('monitor');
		//Trả về danh sách các user trực tuyến trên server cho monitor1.ejs
		io.in('monitor').emit('list-online-monitor', usersSocketIdList);
	});
	...
	socket.on('add user', function (username) {
		...
		if (usersList.indexOf(username) > -1) {
			...
			addedUser = true;
			socket.emit('login-status', {status : true});
	...
	socket.on('send message', function(data) {
		...
		if(sender !== undefined && reciever !== undefined){
			...
			//Tin nhắn sẽ được đẩy tới monitor1.ejs
			io.in('monitor').emit('send-message-monitor', {
				_sender:	usersSocketIdList[sender].AVATAR,
	...
	socket.on('disconnect', function() {
		...
		/* Trả về danh sách các user trực tuyến trên server cho monitor1.ejs */
		io.in('monitor').emit('list-online-monitor', usersList);
